	...
	return withTx(func(tx *sql.Tx) error {
		...
		_, err := tx.Exec(`
			INSERT INTO downloads (
				id, url, source_type, source_key, dest_path, filename, status, total_size, downloaded, url_hash, created_at, paused_at, time_taken, mirrors, chunk_bitmap, actual_chunk_size, file_hash
			) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func LoadState(url string, destPath string) (*types.DownloadState, error) {
	...
	row := db.QueryRow(`
		SELECT id, url, source_type, source_key, dest_path, filename, total_size, downloaded, url_hash, created_at, paused_at, time_taken, mirrors, chunk_bitmap, actual_chunk_size, file_hash
		FROM downloads 
